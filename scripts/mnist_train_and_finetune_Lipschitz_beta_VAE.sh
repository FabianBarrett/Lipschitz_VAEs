...
KL_beta=5.0
saving_tag="continuous_bernoulli_std_dev_better_convergence_beta_vae"
for l_constant in "${l_constants[@]}"
do
	echo "Training model for Lipschitz constant $l_constant..."
	pythonw ./lnets/tasks/vae/mains/train_VAE.py ./lnets/tasks/vae/configs/mnist/fc_beta_VAE_bjorck.json -o model.linear.bjorck_iter=3 model.encoder_mean.l_constant=${l_constant} model.encoder_std_dev.l_constant=${l_constant} model.decoder.l_constant=${l_constant} model.KL_beta=${KL_beta} --saving_tag=$saving_tag
